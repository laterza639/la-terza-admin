...
@Component({
  ...
  <div class="p-6">
    ...
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-2xl font-bold text-gray-900">Bebidas</h1>
      <button
    ...
    <div *ngIf="drinks().length === 0" class="bg-yellow-400 rounded-lg shadow p-4 flex items-center justify-between">
      <div class="flex items-center space-x-4">
        <div>
          <p class="font-medium text-gray-900">Agrega bebidas a la lista!</p>
        </div>
      </div>
    </div>
    ...
    <div class="grid gap-4">
      @for (drink of drinks(); track drink.id) {
        <div class="bg-white rounded-lg shadow p-4 flex items-center justify-between">
          <div class="flex items-center space-x-4">
            <div>
              <h3 class="font-medium text-gray-900">{{ drink.name }}</h3>
            </div>
          </div>
          <div class="flex items-center space-x-2">
            ...
            <button
              (click)="openDeleteModal(drink.id)"
...
export default class DrinkComponent {
  ...
  private loadDrinks() {
    this.drinkService.getAll()
      .subscribe(drinks => this.drinks.set(drinks));
  }

  openCreateModal(drink?: Drink) {
    Swal.fire({
      title: 'Nueva Bebida',
      html: /*html*/`
        <form id="drinkForm" class="space-y-4">
          ...
          <div class="space-y-2">
            <label for="size" class="block text-start text-sm font-medium text-gray-700">Tamaño</label>
            <textarea 
              id="size" 
              name="size"
              class="w-full p-2 border rounded-lg focus:ring-yellow-500 focus:border-yellow-500" 
      ...
      preConfirm: () => {
        const form = document.getElementById('drinkForm') as HTMLFormElement;
        const formData = new FormData();

        // Get form field values
        const name = (form.querySelector('#name') as HTMLInputElement).value;
        const price = (form.querySelector('#price') as HTMLInputElement).value;
        const size = (form.querySelector('#size') as HTMLTextAreaElement).value;
        ...
        if (size.length >= 3) formData.append('size', size);
      ...
      if (result.isConfirmed && result.value) {
        if (drink) {
          this.updateDrinkConfirmation(drink.id, result.value);
        } else {
          this.createDrinkConfirmation(result.value);
        }
      }
    });
  }

  openEditModal(drink: Drink) {
    const imageUrl = drink.img
      ? this.drinkService.getImageUrl(drink.img)
      : '';
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.drinkService.delete(id).subscribe({
          ...
          error: () => Swal.fire('Error', 'No se pudo eliminar la bebida', 'error')
        });
      }
    });
  }

  createDrinkConfirmation(formData: FormData) {
    this.drinkService.create(formData).subscribe({
      next: () => {
        this.loadDrinks();
        Swal.fire({
          title: '¡Éxito!',
      ...
      error: () => Swal.fire('Error', 'No se pudo agregar la bebida', 'error')
    });
  }

  updateDrinkConfirmation(id: string, formData: FormData) {
    this.drinkService.update(id, formData).subscribe({
      next: () => {
        this.loadDrinks();
        Swal.fire({
          title: '¡Éxito!',
      ...
      error: () => Swal.fire('Error', 'No se pudo actualizar la bebida', 'error')
    });
  }
